...
impl RequestFile {
    ...
    fn extract_symbol(&self, content: &str, symbol: &str) -> Result<(String, String)> {
        ...
        if total_lines == 0 { return Err(RequestError::SymbolNotFound(symbol.to_string())); }

        let target_line_idx = lines.iter().position(|l| l.contains(symbol))
            .ok_or_else(|| RequestError::SymbolNotFound(symbol.to_string()))?;

        let context = 5;
        let start_idx = target_line_idx.saturating_sub(context);
        let end_idx = (target_line_idx + context).min(total_lines.saturating_sub(1));
